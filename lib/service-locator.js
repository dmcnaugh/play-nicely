...
debug('Entered module private scope, initialisation')

var ab = require('./angular-bits');

debug('Creating service locator store');
var serviceLocator = {};

var optionsObj = null;

/**
 * method: please - adds a target (object) to the dependency list identified by the given token
 * @param {*} target
 * @param {String} token
 * @returns the target
 */
...
/**
 * method: thankYou - how would you describe it?
...
function thankYou () {
    ...
    if (typeof arguments[0] == 'string') {
        ...
        [].shift.call(arguments); //TODO: maybe should use Array.prototype.shift.call
    } else {
        var $user = null;
    }
    /*****/
    ...
    for(var i= 0, l=tokens.length; i < l; i++) {
        if(optionsObj.strict) assert(serviceLocator.hasOwnProperty(tokens[i]), 'token \"'+tokens[i]+'\" not found');
...
exports = module.exports = function(options) {
    ...
    debug('Entered module Constructor');
    ...
    if(optionsObj == null) {
        optionsObj = options;
        debug('Options set by the first call');
    } else {
        options = optionsObj;
        debug('Options ignored, determined by the first call');
    }

    debug('Options used: ', optionsObj);
